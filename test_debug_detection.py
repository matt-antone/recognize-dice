...
def debug_detection_pipeline(frame: np.ndarray, frame_num: int):
    ...
    print(f"Gray stats: mean={np.mean(gray):.1f}, std={np.std(gray):.1f}")
    ...
    print(f"Enhanced stats: mean={np.mean(enhanced):.1f}, std={np.std(enhanced):.1f}")
    ...
    param_sets = [
        # Original params
        {"dp": 1, "minDist": 30, "param1": 100, "param2": 25, "minRadius": 10, "maxRadius": 80},
        # More sensitive
        {"dp": 1, "minDist": 25, "param1": 80, "param2": 20, "minRadius": 8, "maxRadius": 100},
        # Less sensitive
        {"dp": 1, "minDist": 40, "param1": 120, "param2": 30, "minRadius": 15, "maxRadius": 60},
        # Very sensitive
        {"dp": 1, "minDist": 20, "param1": 60, "param2": 15, "minRadius": 5, "maxRadius": 120},
    ]
    ...
    for i, params in enumerate(param_sets):
        ...
        print(f"Param set {i+1}: Found {circle_count} circles")
        ...
        cv2.imwrite(f"debug_06_circles_params{i+1}_{frame_num}.jpg", debug_img)
        
        if circle_count > best_count:
            best_circles = circles
            best_count = circle_count
    
    print(f"Best result: {best_count} circles")
    ...
    print(f"Found {len(dice_contours)} potential dice contours")
